...
import { registrarPedidoAction } from '@/app/actions/pedidos';
...
export default function CartDrawer() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const dbResponse = await registrarPedidoAction({
        nombre: nombre.trim(),
      ...
      if (!dbResponse.success) {
        alert("Hubo un problema registrando tu pedido en la base de datos. Por favor intenta de nuevo.");
        setIsSubmitting(false);
        return; 
      }

      // 2. CREAMOS EL MENSAJE DE WHATSAPP CON EL ID
      let message = `¡Hola amigos de La Floresta! 👋\n\nSoy *${nombre}*. Mi número de pedido es el *#${dbResponse.pedidoId}* y me gustaría pedir:\n\n`;
      cart.forEach(item => {
      ...
      // 3. LIMPIAMOS LA INTERFAZ ANTES DE SALIR
      // Lo hacemos inmediatamente para que, si el usuario decide volver atrás, el carrito esté vacío.
      clearCart();
      setIsCartOpen(false);
      
      // 4. ABRIMOS WHATSAPP DE FORMA SEGURA (Anti-bloqueo)
      // Usamos href para redirigir la pestaña actual, activando la App de WhatsApp nativa en celulares
      window.location.href = urlWhatsApp;

    } catch (error) {
      console.error("Error al procesar el pedido:", error);
      alert("Ocurrió un error inesperado al conectar con el servidor.");
      setIsSubmitting(false); // Solo quitamos el estado de carga si hubo un error
    } 
  };
